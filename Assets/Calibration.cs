...
public class Calibration {
    ...
    public double calibrateFromCorrespondences(List<Vector3> imagePoints, List<Vector3> worldPoints)
    {
        ...
        var flags = CalibrationFlag.UseIntrinsicGuess; // CalibrationFlag.ZeroTangentDist | CalibrationFlag.FixK1 | CalibrationFlag.FixK2 | CalibrationFlag.FixK3 | CalibrationFlag.FixK4 | CalibrationFlag.FixK5 | CalibrationFlag.FixK6;

        imagePointsCvMat = new CvMat(numPoints, 1, MatrixType.F32C2, imagePoints.ToArray());
        worldPointsCvMat = new CvMat(numPoints, 1, MatrixType.F32C3, worldPoints.ToArray());

        //double[] intrGuess = new double[] { 1.1, 0.4, 3.3 };
        //var ret = GetRotationMatrixFromRotationVector(new CvMat(3, 1, MatrixType.F64C1, intrGuess));

        //CvMat imagePoints = new CvMat(AllPoints, 1, MatrixType.F32C2, allCorners.ToArray());
        int[] counts = new int[] { 7 };
        CvMat pointCounts = new CvMat(1, 1, MatrixType.S32C1, counts);

        //CvMat intrinsic = new CvMat(3, 3, MatrixType.F64C1);
        CvMat distortion = new CvMat(1, 4, MatrixType.F64C1);
        CvMat rotation = new CvMat(1, 3, MatrixType.F64C1);
        CvMat translation = new CvMat(1, 3, MatrixType.F64C1);
    ...
    private CvMat PutImagePointsIntoCVMat(List<Vector3> _imagePositions, int pointsCount)
    {
        ...
        public void DrewTestCalibrateCamera()
        {
            ...
            // 書き込んだファイルを表示
            Debug.Log(File.ReadAllText("camera.xml"));
        }
}